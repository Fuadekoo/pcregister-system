...
<head>
    ...
    <title>{{__('updatepc.Search')}}</title>
...
<body>
    ...
    <h1>{{__('updatepc.UpdatePCRegister')}}</h1>
    ...
    <div class="form-group">
        <label for="pc_name">{{__('updatepc.PCName')}}</label>
        <input type="text" class="form-control" name="pc_name" id="pc_name" value="{{ $pcregister['pc_name'] }}">
    </div>

    <div class="form-group">
        <label for="serial_number">{{__('updatepc.SerialNumber')}}</label>
    ...
    <button type="submit" class="btn btn-primary">{{__('updatepc.Update')}}</button>
</form>
    <br>
    <hr>
    @include('home.footer')
